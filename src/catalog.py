...
def addHaloMass(cat, U, MassConversion):
    """
    ...
    cat['hasM'] = np.zeros(cat.shape[0])

    cat['Mvir'] = np.zeros(cat.shape[0])
    for iObj in range(cat.shape[0]):
        mStellar = cat['Mstellar'][iObj]
        if (mStellar>1.e3) and not np.isnan(mStellar):
            cat['hasM'][iObj] = True
            cat['Mvir'][iObj] = MassConversion.fmStarTomVir(mStellar)


    # for object without a mass, use the mean mass from the others
    if np.sum(cat['hasM'])>0:
        meanMstellar = np.mean(cat['Mstellar'][cat['hasM'].astype('bool')])
        cat['Mstellar'][~cat['hasM'].astype('bool')] = meanMstellar
        #
        meanMvir = np.mean(cat['Mvir'][cat['hasM'].astype('bool')])
        cat['Mvir'][~cat['hasM'].astype('bool')] = meanMvir
    # if no object has a mass, make a random guess, rather than keeping 0
    else:
        cat['Mstellar'] = 2.6e11   # random guess
        cat['Mvir'] = MassConversion.fmStarTomVir(2.6e11)



def addIntegratedTau(cat, U, MassConversion):
    ...
    cat['integratedTau'] = cat['Mvir'] * U.bg.Omega0_b/U.bg.Omega0_m
    ...
    cat['integratedTau'] *= sigma_T / (mpc / U.bg.h)**2

    # divide by (a chi)^2
    cat['integratedTau'] /= (U.bg.comoving_distance(cat['Z']) / (1.+cat['Z']))**2


def addIntegratedKSZ(cat):
    """Integrated kSZ signal: int d^2theta n_e sigma_T (-v/c) Tcmb
    in [muK * sr]
    """
    print("- add integrated kSZ")
    cat['integratedKSZ'] = - cat['integratedTau'] * (cat['vR']/3.e5) * 2.726e6
...
class Catalog(object):
    ...
    def __init__(self, U, MassConversion, name="test", nameLong=None, 
                 pathInCatalog="", rV=1.,  save=False, load=True, nObj=None,
                 pathOut=None):
        ...
        if pathOut is None:
            ...
            self.pathOut = pathOut + self.name
